...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--modes", type=str, default="wmma,mma")
...
def measure_pytorch_baselines(A: torch.Tensor, B: torch.Tensor, warmup: int, iters: int) -> dict[str, float | bool]:
    ...
    baseline_max_abs_diff = (matmul_out - mm_out).abs().max().item()
    baseline_mean_abs_diff = (matmul_out - mm_out).abs().mean().item()
    baselines_match = torch.equal(matmul_out, mm_out)
    cublas_max_abs_diff = (cublas_out - mm_out).abs().max().item()
    cublas_mean_abs_diff = (cublas_out - mm_out).abs().mean().item()
    cublas_close = torch.allclose(cublas_out, mm_out, atol=5e-2, rtol=5e-2)
    ...
    return {
        ...
        "matmul_equals_mm": bool(baselines_match),
        "matmul_vs_mm_max_abs_diff": float(baseline_max_abs_diff),
        "matmul_vs_mm_mean_abs_diff": float(baseline_mean_abs_diff),
        "cublas_close_vs_mm": bool(cublas_close),
        "cublas_vs_mm_max_abs_diff": float(cublas_max_abs_diff),
        "cublas_vs_mm_mean_abs_diff": float(cublas_mean_abs_diff),
    }
...
def run_shape_case(
    ...
    mode_results = [run_mode_case(mode, A, B, baselines=baselines, warmup=warmup, iters=iters) for mode in modes]
